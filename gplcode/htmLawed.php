...
function regReplaceMultiByte($from, $to, $txt) {
  for($i=0; isset($from[$i]) && isset($from[$i]); $i++) {
    $txt = mb_eregi_replace($from[$i], $to[$i], $txt);
  }
...
$t = preg_replace_callback(array('`(<(!\[CDATA\[))(.+?)(\]\]>)`sm', '`(<(!--))(.+?)(-->)`sm', '`(<(pre|script|textarea)[^>]*?>)(.+?)(</\2>)`sm'), 'hl_aux2', $t);
$t = regReplaceMultiByte(array('(<\w[^>]*(?<!/)>)\s+', '\s+', '(<\w[^>]*(?<!/)>) '), array(' \1', ' ', '\1'), $t);
if(($w = strtolower($w)) == -1){
